...
const WorkerList = ({ workers = [], loading = false, error = null }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {workers.map((worker) => {
        const name = worker.user ? `${worker.user.firstName} ${worker.user.lastName}` : '';
        const role = worker.specialty || 'Professional';
        ...
        return (
          <div key={worker.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow p-6 border border-gray-100">
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
              <span className="text-lg font-bold text-gray-900">${price}<span className="text-sm text-gray-500 font-normal">/hr</span></span>
              <Link to={`/workers/${worker.id}`} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-700 transition-colors">
                View Profile
              </Link>
            </div>
          </div>
